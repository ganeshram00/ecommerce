...
import { FaTags, FaDollarSign, FaClipboard, FaLaptop, FaImage } from "react-icons/fa"; // React Icons
...
import { getSingleProducts, updateProducts } from "../../redux/slice/productSlicer"; // Make sure you have the action for updating product
import Loader from "../Loader/Loader";

const UpdateProducts = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { id } = useParams(); // To fetch the product ID from the URL params
    console.log(id);
...
// console.log(product);
    ...
    useEffect(() => {
        if (product) {
            ...
            setImagesPreview(product.images.map(img => img.url)); // Assuming images in `product.images` contain URLs
        }
    }, [product]);
    ...
    const updateProductSubmitHandler = (e) => {
        ...
        images.forEach((image) => {
            myForm.append("images", image); // Ensure images key is used correctly
        });

        dispatch(updateProducts({ id, myForm })); // Dispatch the action to update product
    };
        ...
        <Fragment >
            <div className="flex">
                ...
                <div className="flex-1 p-6 space-y-6 bg-white rounded-lg shadow-md ">
                    <form
                        className="space-y-6 block md:w-[80%] m-auto shadow-lg p-4"
                        encType="multipart/form-data"
                        onSubmit={updateProductSubmitHandler}
                    >
                        <h1 className="text-2xl font-semibold text-gray-800 text-center uppercase underline">Update Product</h1>
